...
export default async (request, context) => {
  ...
  if (method === "GET") {
    try {
      ...
      const contacts = JSON.parse(blob);
      ...
      return new Response(
        JSON.stringify(["Error loading contacts: " + error.message]),
        {
          status: 200,
          headers: {
            "Content-Type": "application/json",
  ...
  if (method === "DELETE") {
      ...
      try {
        ...
        return new Response(
          JSON.stringify({
            ...
            message:
              deletedCount > 0
                ? `All ${deletedCount} contact(s) deleted successfully`
    ...
    try {
      ...
      contacts = contacts.filter(
        (contact) => !idsAsStrings.includes(contact.id.toString()),
      );
      ...
      if (deletedCount === 0) {
        return new Response(
          JSON.stringify({
            success: false,
            message: "No contacts found with provided IDs",
          }),
          {
            status: 404,
            headers: {
